...
In production, doco-cd is used to deploy in a GitOps fashion. The `.doco-cd.yml` file in the root of the repo specifies which deployments are deployed in production. Each deployment is in it's own directory.
...
1. Before making code changes, follow the testing instructions below to verify if the current state of the code runs and all containers are health.
2. After verifying all containers are healthy, make code changes to implement the requested functionality.
...
4. Only submit Pull Requests if app containers are healthy.

## Testing locally:
1. `cd` into the directory of the specific deployment (for example: /media-server)
