...
if ! command -v python3 >/dev/null 2>&1; then
    echo "[sonarr-init] python3 not found; installing..."
    if apk add --no-cache python3; then
        echo "[sonarr-init] python3 installed."
    else
        echo "[sonarr-init] ERROR: Failed to install python3. Aborting."
        exit 1
    fi
fi

exec python3 /opt/sonarr-init-config.py
